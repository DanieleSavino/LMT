...
def extract_lap(sess: TelemetrySession, lap_number: int, color: str = "#ffffff") -> LapData:
    ...
    if wheel_raw is not None:
        def resample_wheel(col: str):
            d = wheel_raw.sort_values("t_rel").drop_duplicates(subset="t_rel")
            x, y = d["t_rel"].to_numpy(), d[col].to_numpy()
            if len(x) < 2:
                return np.full_like(t_on_grid, float(y[0]) if len(y) else np.nan, dtype=float)
        ...
        slips = [
            (resample_wheel(c) * wheel_scale - speed) / np.maximum(np.abs(speed), 1.0)
            for c in WHEEL_COLS
        ]
        ...
        raw_lockup = (min_slip < LOCKUP_SLIP) & (brake > LOCKUP_BRAKE_MIN)
        if abs_ is not None:
            ...
            raw_lockup = raw_lockup & (abs_ < 0.5)
        lockup = raw_lockup.astype(float)

        traction_loss = ((max_slip > TRACTION_LOSS_SLIP) & (throttle > TRACTION_LOSS_THROTTLE_MIN)).astype(float)
